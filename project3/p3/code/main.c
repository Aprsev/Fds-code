...
void find(int start,int end,int k,int** table,int n){//function to find the transportation hub
    ...
    int min_dis = djkstra(start,end,table,n,count,1,k);//use the djkstra algorithm to find the minimal distance
    // printf("%d\n",min_dis);
...
int djkstra(int start,int end,int** table,int n,int* count,int flag,int k){//function to calculate the shortest distance from start to end
    int* dist = (int*)malloc(n*sizeof(int));        //create dist to save the shortest distance from start to i node
    ...
    if(flag){                                       //flag to mark if the function is to simply calculate the distance or find the hubs
        int* dis_e = (int*)malloc(n*sizeof(int));   //create dis_e to save the distance from i to end
        int* his_con = (int*)malloc(n*sizeof(int)); //to mark every node's out data(in shortest path)
        for(int i = 0;i < n;i++){
            dis_e[i] = djkstra(i,end,table,n,count,0,k);//initialize the dis_e by using flag = 0 mode of function djkstra
            his_con[i] = 0;                         //initialize out data as 0
        }
